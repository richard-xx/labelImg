...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(
        ...
        for i in QApplication.screens():
            if i.availableGeometry().contains(saved_position):
                position = saved_position
                break
        self.resize(size)
        self.move(position)
    ...
    def show_bounding_box_from_annotation_file(self, file_path):
        if self.default_save_dir is not None:
            ...
            xml_path = os.path.join(self.default_save_dir, Path(file_path).with_suffix(XML_EXT).name)
            txt_path = os.path.join(self.default_save_dir, Path(file_path).with_suffix(TXT_EXT).name)
            json_path = os.path.join(self.default_save_dir, Path(file_path).with_suffix(JSON_EXT).name)
...
def get_main_app(argv=None):
    ...
    parser.add_argument("image_dir", nargs="?", type=Path, help="")
    parser.add_argument(
        "class_file",
    ...
    parser.add_argument("save_dir", nargs="?", type=Path, help="")
    dialog = argparseqt.gui.ArgDialog(parser, "LabelImg")
    dialog.setValues(vars(parser.parse_args()))
    dialog.exec()
    if dialog.result() != QDialog.DialogCode.Accepted:
        logger.debug(dialog.result())
        sys.exit(dialog.result())
    # args = dialog.getValues()
    args = argparse.Namespace(**dialog.getValues())
    args.image_dir = args.image_dir and Path(args.image_dir).absolute().resolve().as_posix()
    args.class_file = args.class_file and Path(args.class_file).absolute().resolve().as_posix()
    args.save_dir = args.save_dir and Path(args.save_dir).absolute().resolve().as_posix()
