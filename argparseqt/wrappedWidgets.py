...
class FileChooser(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.browseButton.setText("选择")

        self.layout().addWidget(self.textBox)
        self.layout().addWidget(self.browseButton)

        self.browseButton.clicked.connect(self.choose_type)

        self.clear = self.textBox.clear

    def choose_type(self):
        msgBox = QtWidgets.QMessageBox(self)
        msgBox.setText("文件 or 目录 ?")
        msgBox.setIcon(QtWidgets.QMessageBox.Icon.Question)
        browse_file_button = msgBox.addButton("选择文件", QtWidgets.QMessageBox.ButtonRole.ActionRole)
        browse_dir_button = msgBox.addButton("选择目录", QtWidgets.QMessageBox.ButtonRole.ActionRole)
        msgBox.addButton(QtWidgets.QMessageBox.StandardButton.Cancel)

        browse_file_button.clicked.connect(self._browse_file)
        browse_dir_button.clicked.connect(self._browse_dir)

        msgBox.exec()

    def _browse_dir(self):
        selectedFile = QtWidgets.QFileDialog.getExistingDirectory(
            self, "Open dir", str(self.value().parent)
        )
    ...
    def _browse_file(self):
        selectedFile = QtWidgets.QFileDialog.getOpenFileName(
            self, "Open file", str(self.value().parent)
        )
